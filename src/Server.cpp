...
long get_current_time_ms(){
   auto time_now = std::chrono::system_clock::now();
        auto now_in_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(time_now);
...
void handle_connect(int client_fd , int argc, char ** argv){
  std  :: unique_ptr<In_Memory_Storage> key_value_storage {std::make_unique<In_Memory_Storage>()}; 
  for(int i = 1 ;i < argc ;i += 2){
    if(i +1 < argc ){
      std::string key = argv[i];
      std::string value = argv[i+1];
        long current_time_in_ms = get_current_time_ms();
        key_value_storage->set(key.substr(2),value,current_time_in_ms +999999999999 );
    }
  }
  while(true){
  char msg[1024] = {} ;
  ...
  std::lock_guard<std::mutex> guard(mutex_guard);
   ...
   std :: unique_ptr<Redis> parser{std::make_unique<Redis>(header)} ;
    ...
    for(auto it : parser_list) std :: cout << it << std::endl;
  // guard.lock();
    if(parser_list[0] == "PING"){
      response = "+PONG\r\n";
       ...
       // std :: cout <<  "Time is " << current_time_in_ms << std :: endl;
        //std :: cout <<  "Time is " << current_time_in_ms + stoi(parser_list.back()) << std :: endl;
        response = "+OK\r\n";
        if(parser_list.size() > 3) key_value_storage->set(parser_list[1], parser_list[2],current_time_in_ms + stoi(parser_list.back()));
        else key_value_storage->set(parser_list[1], parser_list[2],current_time_in_ms + 999999999999);
...
int main(int argc, char **argv) {
  ...
  while(true){
   ...
   int client_fd = accept(server_fd, (struct sockaddr *) &client_addr, (socklen_t *) &client_addr_len);
    ...
    std::thread th(handle_connect,client_fd,argc,argv) ;
