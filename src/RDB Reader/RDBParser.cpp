...
RDBParser ::read_path(std::string endpoint) {
  ...
  while (id < all.size() && (int)(all[id]) != -1) {
    ...
    if (!experity.empty())
      merge.push_back(std::to_string(current_time_in_ms));
    else
      merge.push_back("-1");
